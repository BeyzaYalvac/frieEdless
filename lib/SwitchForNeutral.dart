...
class SwitchForNeutralPage extends StatefulWidget {
  const SwitchForNeutralPage({Key? key}) : super(key: key);

  @override
  _SwitchForNeutralPageState createState() => _SwitchForNeutralPageState();
}

class _SwitchForNeutralPageState extends State<SwitchForNeutralPage> {
    ...
    try {
      ...
      setState(() {
        persons = documents.map((doc) => doc.data() as Map<String, dynamic>).where((person) =>
            ['extrovert', 'neutral', 'introverted'].contains(person['person_personality'])).toList();
      });

      print("Fetched ${persons.length} persons");
    } catch (e) {
      print("Failed to fetch persons: $e");
    }
  }
