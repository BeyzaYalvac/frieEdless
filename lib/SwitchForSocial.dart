...
class SwitchForSocialPage extends StatefulWidget {
  const SwitchForSocialPage({Key? key}) : super(key: key);

  @override
  _SwitchForSocialPageState createState() => _SwitchForSocialPageState();
}

class _SwitchForSocialPageState extends State<SwitchForSocialPage> {
  ...
  Future<void> _fetchPersons() async {
    ...
    setState(() {
      persons = documents.map((doc) => doc.data() as Map<String, dynamic>).where((person) =>
          ['social', 'extrovert'].contains(person['person_personality'])).toList();
    });
  }
