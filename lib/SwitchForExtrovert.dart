...
class SwitchForExtrovertPage extends StatefulWidget {
  const SwitchForExtrovertPage({Key? key}) : super(key: key);

  @override
  _SwitchForExtrovertPageState createState() => _SwitchForExtrovertPageState();
}

class _SwitchForExtrovertPageState extends State<SwitchForExtrovertPage> {
  ...
  Future<void> _fetchPersons() async {
    ...
    setState(() {
      persons = documents.map((doc) => doc.data() as Map<String, dynamic>).where((person) =>
          ['social', 'extrovert'].contains(person['pepersonalityrson_'])).toList();
    });
  }
